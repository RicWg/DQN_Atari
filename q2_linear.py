...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder(shape=[None, img_height, img_width, nchannels*config.state_history],
                                dtype=tf.uint8)
        self.a = tf.placeholder(shape=[None], dtype=tf.int32)
        self.r = tf.placeholder(shape=[None], dtype=tf.float32)
        self.sp = tf.placeholder(shape=[None, img_height, img_width, nchannels*config.state_history],
                                 dtype=tf.uint8)
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        #pass
        with tf.variable_scope(scope, reuse=reuse) as get_q:
            flat_s = tf.layers.flatten(state)
            out = layers.fully_connected(inputs=flat_s, num_outputs=num_actions, activation_fn=None)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        #pass
        var_q_scope = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=q_scope)
        var_target_q = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=target_q_scope)
        new_var = [tf.assign(var_target_q[i], var_q_scope[i]) for i in range(len(var_q_scope))]
    ...
    def add_optimizer_op(self, scope):
        ...
        gradients, v = list(zip(*optimizer.compute_gradients(self.loss, var)))
